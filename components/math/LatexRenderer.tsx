    ...
    <div className={`space-y-1 ${className || ''}`}>
      {lines.map((line, i) => {
        ...
        if (isBoldHeader) {
          ...
          return (
            <div key={i} className="font-bold text-[#2D4A3E] mt-3">
        ...
        if (numberedMatch) {
          return (
            <div key={i} className="flex gap-2 items-start">
              <span className="text-[#2D4A3E] font-bold min-w-[1.5rem]">{numberedMatch[1]}.</span>
        ...
        if (line.startsWith('## ')) {
          return (
            <h2 key={i} className="text-lg font-bold text-[#1A1A1A] mt-4 mb-2">
        ...
        if (line.startsWith('### ')) {
          return (
            <h3 key={i} className="font-semibold text-[#2D4A3E] mt-3 mb-1">
        ...
        // Horizontal rule
        if (line.trim() === '---') {
          return <hr key={i} className="border-[#E0E0DA] my-3" />;
        }
